...
const OrderDetailPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <button onClick={() => navigate(-1)} className="flex items-center space-x-2 text-gray-500 mb-6 hover:text-black">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-black uppercase">Đơn hàng #{order.order_id.slice(0, 8)}</h1>
          <p className="text-sm text-gray-500">Ngày đặt: {formatDate(order.created_at)}</p>
        </div>
        <div className={`px-4 py-2 text-sm font-bold rounded border ${getStatusColor(order.status)}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-gray-50 p-6 rounded-lg">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
               <p className="text-sm italic text-gray-500">Ghi chú: {order.note || "Không có"}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="border rounded-lg overflow-hidden">
              {order.items.map((item) => (
                <div key={item.item_id} className="flex gap-4 p-4 border-b last:border-0 bg-white">
                  <img src={item.thumbnail} alt={item.product_name} className="w-20 h-20 object-cover bg-gray-100" />
                  <div className="flex-1">
                    <p className="font-bold text-sm uppercase">{item.product_name}</p>
                    <p className="text-xs text-gray-500">Phân loại: {item.size} / {item.color}</p>
                    <p className="text-xs text-gray-500">x{item.quantity}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-medium text-sm">{formatCurrency(item.price_at_purchase)}</p>
                    <p className="font-bold text-sm mt-1">{formatCurrency(item.total_item_price)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-gray-50 p-6 rounded-lg">
            ...
            <p className="text-sm font-medium">Phương thức: <span className="font-bold">{order.payment_method}</span></p>
            <p className="text-sm">Trạng thái: 
               <span className={order.payment_status === 'paid' ? 'text-green-600 font-bold ml-1' : 'text-gray-600 ml-1'}>

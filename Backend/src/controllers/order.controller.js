...
const previewOrder = asyncHandler(async (req, res) => {
  ...
  cartItems.forEach(item => {
    if (item.variant && item.variant.product) {
      const price = parseFloat(item.variant.product.base_price) + (parseFloat(item.variant.price_modifier) || 0);
  ...
  if (address_id) {
    const address = await db.Address.findOne({ where: { address_id, user_id: userId } });
    if (address) {
  ...
  if (voucher_code) {
    ...
    if (!voucher) {
      ...
      if (merchandiseSubtotal < parseFloat(voucher.min_order_value)) {
        voucherInfo = { valid: false, message: `Đơn hàng phải từ ${parseFloat(voucher.min_order_value).toLocaleString()}đ` };
      } else {
        if (voucher.discount_type === 'percent') {
          discountAmount = merchandiseSubtotal * (parseFloat(voucher.discount_value) / 100);
        } else {
          discountAmount = parseFloat(voucher.discount_value);
        }

        if (discountAmount > merchandiseSubtotal) discountAmount = merchandiseSubtotal;
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of cartItems) {
    if (!item.variant || !item.variant.product) {
      return res.status(400).json({ message: 'Có sản phẩm lỗi trong giỏ hàng, vui lòng kiểm tra lại.' });
    }
  ...
  let voucherId = null;
    ...
    if (voucher) {
      ...
      if (merchandiseSubtotal >= parseFloat(voucher.min_order_value)) {
        ...
        return res.status(400).json({ message: `Đơn hàng chưa đạt giá trị tối thiểu để dùng mã này!` });
      }
    } else {
...
const markOrderAsPaid = asyncHandler(async (req, res) => {
  ...
  if (order.payment_status === 'paid') {
    return res.status(200).json({ message: 'Đơn hàng này đã được thanh toán rồi' });
  }
...
const cancelOrder = asyncHandler(async (req, res) => {
  ...
  if (!['pending', 'processing'].includes(order.status)) {
    return res.status(400).json({ message: 'Đơn hàng đã được giao cho vận chuyển, không thể hủy!' });
  }

  await db.sequelize.transaction(async (t) => {
    order.status = 'cancelled';
    order.note = order.note ? `${order.note} | Lý do hủy: ${reason || 'Không có'}` : `Lý do hủy: ${reason || 'Không có'}`;
    ...
    for (const item of order.items) {
      ...
      const variant = await db.ProductVariant.findByPk(item.variant_id, { transaction: t });
      if (variant) {
...
const getOrderDetail = asyncHandler(async (req, res) => {
  ...
  const formattedItems = order.items.map(item => {
    ...
    if (item.variant) {
      ...
      else if (item.variant.product && item.variant.product.media && item.variant.product.media.length > 0) {
...
export default {
  previewOrder,createOrder,markOrderAsPaid,getOrderHistory,cancelOrder,getOrderDetail
};